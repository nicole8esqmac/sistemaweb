...
@section('title','Registro de cuentas y auxiliares')
...
<!-- Content -->
    {{-- <div id="content"> --}}
      <div class="container">
          <section class="py-5">
            <h1 class="font-weight-bold mb-0" align="center">LISTADO DE SALDO INICIAL</h1>
 ...
 <!-- MENSAJE ALERTA EXITOSA -->
  ...
  <div class="container-fluid">
    <div class="block mb-8">
        <a href="{{ route('saldoinicialAdmin.create') }}" class="btn btn-primary"><i class="bi bi-plus-circle-fill"></i>&nbsp; Añadir</a>
    </div>
    <br>
    <div class="card">
        <div class="card-body">
  ...
  <section>
          <table id="tablaSaldoInicial"  class="table table-dark" style="width:100%" >
            <thead>
            ...
            <tbody>
                @forelse ($saldoInicial as $dato)
                <tr class="table-secondary  roboto-medium">
                    <td>{{ $dato->id}}</td>
                    <td>{{ $dato->fecha_registro}}</td>
                    {{-- manejo_proyecto es la funcion llamada del modelo BeneficiarioProyecto --}}
                    <td>{{ $dato->sIEmpresaAdmin->nombre_empresa}} </td>
                    <td>{{ $dato->descripcion}}</td>
                    <td>Q {{ $dato->total}}</td>
                    <td>
                        <form action="{{ route("saldoinicialAdmin.show", $dato->id) }}" method="get">
                    ...
                    <td>
                        <form action="{{ route("saldoinicialAdmin.destroy", $dato->id) }}" method="POST" onsubmit="return confirm('¿Desea cancelar la fila seleccionada ?');">
...
<script>
$(function(){
    $("#tablaSaldoInicial").DataTable(dataTableOpciones);
