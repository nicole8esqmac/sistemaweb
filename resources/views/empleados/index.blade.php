...
<!-- Content -->
    {{-- <div id="content"> --}}
      <div class="container">
          <section class="py-5">
            <h2 class="font-weight-bold mb-0"><i class="bi bi-file-earmark-text-fill"></i>&nbsp; LISTA DE EMPLEADOS</h2>
            <p class="lead text-muted">Revisa el contenido</p>
          </section>

	<div class="container-fluid">
	    <ul class="full-box list-unstyled page-nav-tabs">
            <li><a class="active" href="{{ route("empleados.index") }}"><i class="bi bi-file-earmark-text-fill"></i> LISTA DE EMPLEADOS</a></li>
            <li><a href="{{ route("users.index") }}"><i class="bi bi-file-earmark-text-fill"></i> LISTA DE USUARIOS</a></li>
        </ul>
	</div>
 ...
 <!-- MENSAJE ALERTA EXITOSA -->
     ...
     <div class="container-fluid">
        <div class="block mb-8">
            <a href="{{ route('empleados.create') }}" class="btn btn-primary"><i class="bi bi-plus-circle-fill"></i>&nbsp; Añadir</a>
        </div>
        <br>
        <div class="card">
            <div class="card-body">
      <section>
          <div class="table-responsive">
              <table id="tableEmpleado"  class="table table-dark" style="width:100%">
                    <thead>
                    ...
                    <tbody>
                        @forelse ($empleado as $dato)
                        <tr class="table-secondary roboto-medium">
                            <td>{{ $dato->id}} </td>
                            <td>{{ $dato->dpi}} </td>
                            <td>{{ $dato->nombre }} {{ $dato->apellido }}</td>
                            <td>{{ $dato->sexo}} </td>
                            <td>Q {{ $dato->salario}} </td>
                            <td class="text-center">
                              <form action="{{ route("empleados.show", $dato->id) }}" method="get">
                                <button class="btn btn-warning btn-sm"><i class="bi bi-file-text-fill"></i></button>
                              </form>
                            </td>
                            <td class="text-center">
                              <form action="{{ route("empleados.edit", $dato->id) }}" method="get">
                                <button class="btn btn-secondary btn-sm"><i class="bi bi-pencil-fill"></i></button>
                              </form>
                            </td>
                            <td class="text-center">
                              <form action="{{ route("empleados.destroy", $dato->id) }}" method="POST" onsubmit="return confirm('¿Esta seguro de eliminar este registro?');">
                        ...
                        @empty
                        <h2>NO HAY REGISTROS EN LA BASE DATOS</h2>
...
<script>
$(function(){
    $("#tableEmpleado").DataTable(dataTableOpciones);
