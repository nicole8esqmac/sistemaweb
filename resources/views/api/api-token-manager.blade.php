<div>
    ...
    <x-form-section submit="createApiToken">
        ...
        <x-slot name="description">
            {{ __('API tokens permitir que servicios de terceros se autentiquen con nuestra aplicación en su nombre.') }}
        </x-slot>

        <x-slot name="form">
            <!-- Token Name -->
            <div class="col-span-6 sm:col-span-4">
                <x-label for="name" value="{{ __('Nombre Token') }}" />
    ...
    @if ($this->user->tokens->isNotEmpty())
        ...
        <div class="mt-10 sm:mt-0">
            <x-action-section>
                <x-slot name="title">
                    {{ __('Gestionar tokens API') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('Puede eliminar cualquiera de sus fichas existentes si ya no las necesita.') }}
                </x-slot>
                                ...
                                <div class="flex items-center ml-2">
                                    @if ($token->last_used_at)
                                        <div class="text-sm text-gray-400">
                                            {{ __('Última vez utilizado') }} {{ $token->last_used_at->diffForHumans() }}
                                        </div>
                                    @endif
    ...
    <x-dialog-modal wire:model="displayingToken">
        ...
        <x-slot name="content">
            <div>
                {{ __('Por favor, copie su nuevo token API. Por su seguridad, no se volverá a mostrar.') }}
            </div>
