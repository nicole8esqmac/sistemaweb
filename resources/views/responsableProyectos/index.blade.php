...
@section('title','Proyecto-Beneficiario')
...
<!-- Content -->
      ...
      <div class="container">
          <section class="py-5">
            <h2 class="font-weight-bold mb-0"><i class="bi bi-file-earmark-text-fill"></i>&nbsp; Lista Responsable del Proyecto</h2>
 ...
 <!-- MENSAJE ALERTA EXITOSA -->
     ...
     <div class="container-fluid">
        <div class="block mb-8">
            <a href="{{ route('responsableProyectos.create') }}" class="btn btn-primary"><i class="bi bi-plus-circle-fill"></i>&nbsp; Añadir</a>
        </div>
        <br>
      <section>
          <div class="table-responsive">
              <table id="tablaResponsableProyecto" class="table table-dark" style="width:100%">
                    <thead>
                    ...
                    <tbody>
                        @forelse ($responsableProyectos as $dato)
                        <tr class="table-secondary  roboto-medium">
                            <td>{{ $dato->id}} </td>
                            <td>{{ $dato->fecha_hora}}</td>
                            {{-- manejo_proyecto es la funcion llamada del modelo ResponsableProyecto --}}
                            <td>{{ $dato->manejo_proyecto->titulo}} </td>
                            <td>{{ $dato->tipoDocumento}}: {{ $dato->documento}}</td>
                            <td>{{ $dato->nombre}} {{ $dato->apellido}}</td>
                            <td>{{ $dato->direccion}}</td>

                            <td class="text-center">
                            <form action="{{ route("responsableProyectos.show", $dato->id) }}" method="get">
                              <button class="btn btn-warning btn-sm"><i class="bi bi-file-text-fill"></i></button>
                            </form>
                          </td>
                          <td class="text-center">
                            <form action="{{ route("responsableProyectos.edit", $dato->id) }}" method="get">
                              <button class="btn btn-secondary btn-sm"><i class="bi bi-pencil-fill"></i></button>
                            </form>
                          </td>
                          {{-- <td class="text-center">
                            <form action="{{ route("responsableProyectos.destroy", $dato->id) }}" method="POST" onsubmit="return confirm('Esta seguro?');">
...
<script>
$(function(){
    $("#tablaResponsableProyecto").DataTable(dataTableOpciones);
